...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train, y_train), 1):
    Xf_tr,  yf_tr  = X_train[tr_idx].to(DEVICE), y_train[tr_idx].to(DEVICE)
    Xf_val, yf_val = X_train[val_idx].to(DEVICE), y_train[val_idx].to(DEVICE)

    fold_loader = DataLoader(TensorDataset(Xf_tr, yf_tr),
                             batch_size=TRAIN_PARAMS['batch_size'], shuffle=True)
    fold_model = SPDNetClassic().to(DEVICE)
    fold_opt   = torch.optim.Adam(fold_model.parameters(),
                                  lr=TRAIN_PARAMS['lr'],
                                  weight_decay=TRAIN_PARAMS['weight_decay'])
    fold_crit  = nn.CrossEntropyLoss()

    best_val_loss = float('inf'); best_fold_state = None; wait = 0
    ...
    fold_model.eval()
    with torch.no_grad():
        preds = fold_model(Xf_val).argmax(dim=1).cpu().numpy()
    score = balanced_accuracy_score(yf_val.cpu().numpy(), preds)
    cv_scores.append(round(float(score), 4))
    print(f"  Fold {fold}: bAcc = {score:.4f}")
    del fold_model, Xf_tr, yf_tr, Xf_val, yf_val
...
ax.bar(folds_x, cv_scores, color='steelblue', alpha=0.8, label='bAcc per fold')
ax.axhline(cv_scores.mean(), color='red', linestyle='--', linewidth=1.5,
           label=f'Media = {cv_scores.mean():.4f}')
ax.fill_between(folds_x, cv_scores.mean()-cv_scores.std(),
...
X_tr,    X_val_fin = X_train[:val_split].to(DEVICE), X_train[val_split:].to(DEVICE)
y_tr,    y_val_fin = y_train[:val_split].to(DEVICE), y_train[val_split:].to(DEVICE)
X_test_d, y_test_d = X_test.to(DEVICE), y_test.to(DEVICE)

train_loader = DataLoader(TensorDataset(X_tr, y_tr),
                          batch_size=TRAIN_PARAMS['batch_size'], shuffle=True)
test_loader  = DataLoader(TensorDataset(X_test_d, y_test_d),
                          batch_size=TRAIN_PARAMS['batch_size'], shuffle=False)

model     = SPDNetClassic().to(DEVICE)
...
best_val_loss = float('inf'); best_state = None; wait = 0; stopped_epoch = MAX_EPOCHS
...
print(f"  [OK] Best model caricato (best_val_loss={best_val_loss:.6f})")

fig, ax = plt.subplots(figsize=(8, 4))
epochs_x = range(1, len(train_loss_history) + 1)
ax.plot(epochs_x, train_loss_history, color='steelblue',  linewidth=1.8, label='Train Loss')
ax.plot(epochs_x, val_loss_history,   color='darkorange', linewidth=1.8,
...
with torch.no_grad():
    for Xb, yb in test_loader:
        ...
        all_preds.extend(preds); all_true.extend(yb.cpu().numpy())
...
params_log = {
    ...
    'n_ictal_train': int(y_tr.cpu().sum().item()),
    'n_ictal_val':   int(y_val_fin.cpu().sum().item()),
...
with open(csv_path, 'w', newline='') as f:
    ...
    writer.writerow({
        ...
        "cv_mean":       round(float(cv_scores.mean()), 4),
        "cv_std":        round(float(cv_scores.std()),  4),
        "final_loss":    round(best_val_loss, 6),
        "stopped_epoch": stopped_epoch,
    })
